...
export const SOURCES = [
  ...
  {
    id: "history", label: "Server RAM (mirrored to Firestore)", keyless: true,
    endpoints: ["/api/history", "/api/observations", "/api/trades"],
...
export const PANELS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { field: "z", source: "server samples, computed in app", freshness: "15s samples, needs 8", formula: "(current - mean) / stdev over the sample series" },
      { field: "x (multiple)", source: "server samples, computed in app", freshness: "15s samples, needs 8", formula: "current / mean" },
    ],
  },
  {
...
export const PIPELINE = [
  ...
  { stage: "5. Persist (server)", detail: "Firestore mirrors those raw series so they survive sleeps, deploys and restarts. Optional - off without credentials.",
  ...
  { stage: "7. Calculate (app)", detail: "calculations/asset-detail.js turns that row plus the baselines into twelve components, two modifiers, risk flags, a score and a stage. This is the only place a score exists.",
    cadence: "per poll, in the browser" },
